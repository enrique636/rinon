...
export default function CierresPerimetralesPage() {
  const [form, setForm] = useState({ nombre: "", empresa: "", telefono: "", email: "", ciudad: "", tipo: "", metros: "", instalacion: "", comentarios: "" });
          ...
          <div className="bg-slate-900 rounded-2xl p-8 md:p-10">
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <select required value={form.tipo} onChange={e => setForm(p => ({ ...p, tipo: e.target.value }))}
                ...
                <option value="" className="text-slate-500">Tipo de proyecto *</option>
                {["Obra en construcción", "Bodega / Galpón", "Industria", "Condominio", "Estacionamiento", "Terreno / Parcela", "Faena minera / forestal", "Otro"].map(o => <option key={o} value={o}>{o}</option>)}
              </select>
              ...
              <textarea placeholder="Comentarios, detalles del terreno, tipo de suelo, referencia..." value={form.comentarios} onChange={e => setForm(p => ({ ...p, comentarios: e.target.value }))}
